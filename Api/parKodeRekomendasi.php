...
try {		
	...
	$rs_get_data = $param->kode_rek_data_viewlist();
	$count_data = $rs_get_data->RecordCount();
	if($count_data!=0){
		while($arr_get_data = $rs_get_data->FetchRow()){
			$content[] = array(
						"kode_rek_id" => $arr_get_data['kode_rek_id'],
						"kode_rek_code" => $arr_get_data['kode_rek_code'],
						"kode_rek_desc" => $arr_get_data['kode_rek_desc'],
						"kode_rek_del_st" => $arr_get_data['kode_rek_del_st']
					);
		}
