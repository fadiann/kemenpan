...
try {		
	...
	$rs_get_data = $param->ref_audit_data_viewlist();
	$count_data = $rs_get_data->RecordCount();
	if($count_data!=0){
		while($arr_get_data = $rs_get_data->FetchRow()){
			$content[] = array(
						"ref_audit_id" => $arr_get_data['ref_audit_id'],
						"ref_audit_id_kategori" => $arr_get_data['ref_audit_id_kategori'],
						"ref_audit_nama" => $arr_get_data['ref_audit_nama'],
						"ref_audit_desc" => $arr_get_data['ref_audit_desc'],
						"ref_audit_attach" => $arr_get_data['ref_audit_attach'],
						"ref_audit_del_st" => $arr_get_data['ref_audit_del_st'],
						"kategori_ref_name" => $arr_get_data['kategori_ref_name']
					);
		}
