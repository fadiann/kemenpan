...
class userm {
	...
	function users_count($user_id = "", $key_search = "", $val_search = "", $all_field = "") {
		...
		if($val_search!=""){
			if($key_search!=""){ 
				if($key_search=='auditor_name'){
					$condition2 = " and (auditor_name like '%".$val_search."%' or pic_name like '%".$val_search."%') ";
				}else{
	...
	function users_viewlist($key_search, $val_search, $all_field, $offset, $num_row) {
		...
		if($val_search!=""){
			if($key_search!="") {
				if($key_search=='auditor_name'){
					$condition = " and (auditor_name like '%".$val_search."%' or pic_name like '%".$val_search."%') ";
				}else{
		...
		$sql = "select user_id, user_username, group_name, case user_login_as when '1' then auditor_name when '2' then pic_name end as login_as, (select count(*) from login_expired where login_exp_id_user = user_id) as status_login
				from user_apps
				left join par_group on user_id_group = group_id
				left join auditee_pic on user_id_ekternal = pic_id
				left join auditor on user_id_internal = auditor_id
	...
	function users_view_data($user_id) {
		$sql = "select user_id, user_username, user_id_group, user_login_as, user_id_internal, user_id_ekternal, user_status
				from user_apps
	...
	function users_add($name, $password, $group_id, $login_as, $internal_id, $eksternal_id) {
		$id_user = $this->uniq_id ();
		$pass = md5 ( crypt ( $password, md5 ( $name ) ) );
		$sql = "insert into user_apps (user_id, user_username, user_password, user_id_group, user_login_as, user_id_internal, user_id_ekternal, user_status, user_create_by, user_create_date) VALUES ('" . $id_user . "', '" . $name . "', '" . $pass . "', '" . $group_id . "', '" . $login_as . "', '" . $internal_id . "', '" . $eksternal_id . "', '1', '" . $this->userId . "', '" . strtotime ( "now" ) . "')"; 
	...
	function users_edit($id_user, $name, $password, $group_id, $login_as, $internal_id, $eksternal_id) {
		...
		$sql = "update user_apps set user_username = '" . $name . "' " . $condition . ", user_id_group = '" . $group_id . "', user_login_as = '" . $login_as . "' , user_id_internal = '" . $internal_id . "', user_id_ekternal = '" . $eksternal_id . "' where user_id = '" . $id_user . "' ";
	...
	function group_add_roles($id, $menu_id) {
		$sql = "insert into role_group_menu (role_menu_group_id, role_menu_menu_id) values ('" . $id . "','" . $menu_id . "')";
		$this->_db->_dbquery ( $sql );
	}
